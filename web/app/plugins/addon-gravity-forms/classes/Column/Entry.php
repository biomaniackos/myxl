...
class Entry extends AC\Column implements ACP\Export\Exportable, ACP\Editing\Editable, ACP\Search\Searchable {
	...
	public function get_entry_value( $id ) {
		...
		$field = GFAPI::get_field( $this->get_form_id(), $this->get_field_id() );
		$entry = GFAPI::get_entry( $id );
		$value = isset( $entry[ $this->get_field_id() ] ) ? $entry[ $this->get_field_id() ] : '';
		$columns = GFFormsModel::get_grid_columns( $this->get_form_id() );

		return $field->get_value_entry_list( $value, $entry, $this->get_field_id(), $columns, $form );
	}
	...
	public function editing() {
		return ( new Editing\Model\EntryFactory )->create( $this, $this->get_field_id(), $this->get_field() );
	}
